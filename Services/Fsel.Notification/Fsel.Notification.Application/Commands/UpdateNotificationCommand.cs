...
namespace Fsel.Notification.Application.Commands
{
    ...
    public class UpdateNotificationCommandHandler : IRequestHandler<UpdateNotificationCommand, MethodResult<NotificationMessageModel>>
    {
        ...
        public async Task<MethodResult<NotificationMessageModel>> Handle(UpdateNotificationCommand request, CancellationToken cancellationToken)
        {
            ...
            var listNotificationUpdate = _notificationsRepository.Queryable.Where(x => userIds.Contains(x.UserId!.Value)).ToList();
            if (userIds.Count > 0)
            {
                foreach (var item in listNotificationUpdate)
                {
                    item.Message = request.Message;
                    listNotificationMessage.Add(item);
                }
            }
        ...
        /// <returns></returns>
        private async Task<List<Guid>> FilterListUser(UpdateNotificationCommand request, CancellationToken cancellationToken)
        {
