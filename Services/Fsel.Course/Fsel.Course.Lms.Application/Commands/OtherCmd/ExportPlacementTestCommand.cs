...
namespace Fsel.Course.Lms.Application.Commands.OtherCmd
{
    ...
    public class ExportPlacementTestCommand : SendMailFinishPTModel, IRequest<MethodResult<byte[]>>
    {
    }

    public class ExportPlacementTestCommandHandler : IRequestHandler<ExportPlacementTestCommand, MethodResult<byte[]>>
    {
        ...
        public ExportPlacementTestCommandHandler(IUserService userService, ITestGroupResultRepository testGroupResultRepository, ISkillRepository skillRepository, ILevelRepository levelRepository, AppSetting appSetting, IMediator mediator, ISenderService senderService)
        {
        ...
        public async Task<MethodResult<byte[]>> Handle(ExportPlacementTestCommand request, CancellationToken cancellationToken)
        {
            ...
            var skillHtml = await SendMailHelper.GetTemplateFromPath(AppDomain.CurrentDomain.BaseDirectory, SendMailSetting.CourseSkillPdf, cancellationToken);
            ...
            var pdfResult = await _senderService.ExportPdfFromTemplate(new ExportPdfByTemplateCommandModel()
            {
                Params = sendStudentPT,
                Template = EnumSenderTemplate.StudentCompletePTToPdf
            });

            methodResult.Result = pdfResult.Content?.Result;
            return methodResult;
        }
