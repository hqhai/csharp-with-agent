...
namespace Fsel.Course.Lms.Application.Commands.StudentRankingEventCmd
{
    ...
    public class SetStudentRankingEventCommand : IRequest<MethodResult<IList<CompetitionStudentProgressModel>>>
    {
        public Guid UserId { get; set; }
    }

    public class SetStudentRankingEventCommandHandler : IRequestHandler<SetStudentRankingEventCommand, MethodResult<IList<CompetitionStudentProgressModel>>>
    {
        ...
        private readonly StudentRankingEventsPublisher _studentRankingEventsPublisher;
        ...
        public SetStudentRankingEventCommandHandler(ICourseResultRepository courseResultRepository, ManagerProgressHelper managerProgressHelper, ICourseRepository courseRepository, ITrainingService trainingService, IVideoResultRepository videoResultRepository, IHomeWorkResultRepository homeWorkResultRepository, IFinalTestResultRepository finalTestResultRepository, IClassForumResultRepository classForumResultRepository, StudentRankingEventsPublisher studentRankingEventsPublisher, IUserService userService)
        {
            ...
            _studentRankingEventsPublisher = studentRankingEventsPublisher;
            _userService = userService;
        }

        public async Task<MethodResult<IList<CompetitionStudentProgressModel>>> Handle(SetStudentRankingEventCommand request, CancellationToken cancellationToken)
        {
            ...
            var studentIdByUser = studentResult.Content.Result.Id;
            var studentIds = new List<Guid> { studentIdByUser };

            var courseResultStudent = await _courseResultRepository.Queryable.FirstOrDefaultAsync(x =>
               x.Status != EnumResultStatus.Unfinished &&
               x.Status != EnumResultStatus.New &&
               x.WorkingStatus == EnumWorkingStatus.Active &&
               x.StudentId == studentIdByUser, cancellationToken);

            EnumCourseType courseType = courseResultStudent?.Course?.CourseType ?? EnumCourseType.Academic;

            #region Progress

            var classStudentResults = await _trainingService.GetListClassBySpecificStudentIdsAsync(new GetClassListBySpecificStudentIdsModel { StudentIds = new List<Guid> { studentIdByUser } });
            ...
            var unitTestGroupByStudentId = courseType == EnumCourseType.Ielts ? new List<StudentCompetitionAverageScore>() : videoResultCompetition
            ...
            var skillTestGroupByStudentId = courseType == EnumCourseType.Ielts ? new List<StudentCompetitionAverageScore>() : videoResultCompetition
                .GroupBy(vr => vr.StudentId)
                .AsEnumerable()
                .Select(group =>
                {
            ...
            var finalResults = courseType == EnumCourseType.Ielts ? new List<StudentCompetitionAverageScore>() : CompetitionAverageScores(_finalTestResultRepository, ValueSettings.AcademicStudentResultRatio.FinalTestRatio, studentIds, EnumLearnType.FinalTest);
            ...
            {
                await RankedStudentByScore(finalResult, courseResultStudent, cancellationToken);
            }
        ...
        private async Task RankedStudentByScore(CompetitionStudentProgressModel finalResult, CourseResult courseResuilt, CancellationToken cancellationToken)
        {
            StudentRankingEventModel model = new StudentRankingEventModel
            {
                ...
                OverallScore = finalResult.TotalScore,
            };
