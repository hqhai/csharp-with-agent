...
namespace Fsel.Course.Lms.Application.Commands.MockTestCmd
{
    ...
    public class CreateMockTestAnswerBySectionGroupCommandHandler : IRequestHandler<CreateAnswerBySectionGroupCommand, MethodResult<SectionGroupResultModel>>
    {
        ...
        private async Task<MethodResult<(SkillScores, IList<MockTestAnswer>)>> CreateAnswerAsync(CreateAnswerBySectionGroupCommand request, SectionGroup sectionGroup)
        {
            ...
            if (questions == null || sections == null || sectionTimeCodes == null)
            {
            ...
            if (questions != null && questions.Any())
            {
                foreach (var item in request.Answers)
                {
                    ...
                    if (mockTestAnswer == null)
                    {
                        ...
                        mockTestAnswers.Add(GetMockTestAnswer(answerConfig, correctCount, request));
                    }
                }
            }
        ...
        private async Task<MethodResult<(SkillScores, IList<MockTestAnswer>)>> CreateAnswer(CreateAnswerBySectionGroupCommand request, SectionGroup sectionGroup, IList<Domain.Entities.Section>? sections)
        {
            ...
            if (sections != null && sections.Any())
            {
                foreach (var item in request.Answers)
                {
                    ...
                    if (mockTestAnswer == null)
                    {
                        mockTestAnswers.Add(GetMockTestAnswer(item.Answer, default, request));
                    }
                }
            }
        ...
        private static MockTestAnswer GetMockTestAnswer(object? answer, int correctCount, CreateAnswerBySectionGroupCommand request)
        {
            return new MockTestAnswer
            {
                ...
                SectionQuestionId = request.SectionGroupId
            };
        }
