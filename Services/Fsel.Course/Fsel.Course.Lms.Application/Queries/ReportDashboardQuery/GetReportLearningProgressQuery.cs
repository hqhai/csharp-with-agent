...
namespace Fsel.Course.Lms.Application.Queries.ReportDashboardQuery
{
    ...
    public class GetReportLearningProgressQueryHandler : IRequestHandler<GetReportLearningProgressQuery, MethodResult<DashBoardLearningProgressModel>>
    {
        ...
        public async Task<MethodResult<DashBoardLearningProgressModel>> Handle(GetReportLearningProgressQuery request, CancellationToken cancellationToken)
        {
            ...
            var queryPieChart = new List<bool>();
            foreach (var batche in batches)
            {
                ...
                queryPieChart.AddRange(await query.ToListAsync(cancellationToken));
            }
            ...
            {
                var unitOveralls = new List<DisplayModuleResultModel>();
                foreach (var batche in batches)
                {
                    var datas = await (from baseQ in _courseResultRepository.Queryable
                                       ...
                                       select new DisplayModuleResultModel
                                       {
                                           DisplayOrder = g.Key.Number,
                                           TotalUnitDone = g.Select(x => x.ur).Where(x => x.Status == EnumResultStatus.Done).Select(x => x.Id).Distinct().Count(),
                                       }).ToListAsync(cancellationToken);
                    unitOveralls.AddRange(datas);
                }
            ...
            {
                var lessonOveralls = new List<DisplayModuleResultModel>();
                foreach (var batche in batches)
                {
                    var datas = await (from baseQ in _courseResultRepository.Queryable
                                       join c in _courseRepository.Queryable on baseQ.CourseId equals c.Id
                                       ...
                                       {
                                           DisplayOrder = g.Key,
                                           TotalLessonDone = g.Select(x => x).Where(x => x.Status == EnumResultStatus.Done).Select(x => x.Id).Distinct().Count(),
                                       }).ToListAsync(cancellationToken);
                    lessonOveralls.AddRange(datas);
                }
                reportLearningProgress.OverallLearningProgress = new BaseChartResultModel
                {
                    Type = EnumChartType.LineChart,
        ...
        public class DisplayModuleResultModel
        {
