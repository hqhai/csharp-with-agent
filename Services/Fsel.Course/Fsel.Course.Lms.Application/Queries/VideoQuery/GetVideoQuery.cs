    ...
    public class GetVideoStandaloneQueryHandler : IRequestHandler<GetVideoQuery, MethodResult<VideoModel>>
    {
        ...
        public async Task<MethodResult<VideoModel>> Handle(GetVideoQuery request, CancellationToken cancellationToken)
        {
            ...
            var videoModel = new VideoModel
            {
                ...
                {
                    Id = x.Id,
                    TotalCount = x.TimeCodeExercises.Where(x => !x.IsDeleted && x.Exercise != null).Select(x => x.Exercise).SelectMany(x => x!.ExerciseQuestions.Where(x => !x.IsDeleted && x.Question != null)).OrderBy(x => x!.CreatedDate).Select(m => m.Question).Count(),
                    DisplayTime = x.DisplayTime,
                    ExecutionTime = x.ExecutionTime,
                    TimeCodeType = x.TimeCodeType,
                    VideoId = x.VideoId,
                    Status = x.VideoTimeCodeAnswers.All(x => videoResult != null && x.VideoResultId == videoResult.Id && x.Status == EnumCurrentStatus.Done) ? EnumCurrentStatus.Done : EnumCurrentStatus.Process,
