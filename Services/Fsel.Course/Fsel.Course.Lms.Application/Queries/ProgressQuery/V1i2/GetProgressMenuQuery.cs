...
namespace Fsel.Course.Lms.Application.Queries.ProgressQuery.V1i2
{
    ...
    public class GetProgressMenuQueryHandler : IRequestHandler<GetProgressMenuQuery, MethodResult<ProgressMenuModel>>
    {
        ...
        public async Task<MethodResult<ProgressMenuModel>> Handle(GetProgressMenuQuery request, CancellationToken cancellationToken)
        {
            ...
            var courseDonePost = await (from baseQ in _lessonResultRepository.ReadQueryable
                                        join clr in _classForumResultRepository.ReadQueryable on baseQ.Id equals clr.LessonResultId
                                        where baseQ.CourseResultId == courseResult.Id && clr.Status.HasValue
                                        select baseQ).CountAsync(cancellationToken);

            var countDoneUnit = await _unitResultRepository.ReadQueryable.Where(x => x.CourseResultId == courseResult.Id)
                                                           .Where(x => x.Status == EnumResultStatus.Done)
                                                           .CountAsync(cancellationToken);

            progressMenu.NumberOfUnitDone = countDoneUnit;
            progressMenu.NumberOfPostsCreated = courseDonePost;
        ...
        private async Task<MethodResult<CourseResult>> ValidateAsync(GetProgressMenuQuery request)
        {
            ...
            var courseResult = await _courseResultRepository.ReadQueryable.FirstOrDefaultAsync(x => x.StudentId == student.Id && x.CourseId == request.CourseId);
