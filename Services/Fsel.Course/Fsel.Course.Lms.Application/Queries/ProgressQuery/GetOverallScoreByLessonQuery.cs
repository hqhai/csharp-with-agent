...
namespace Fsel.Course.Lms.Application.Queries.ProgressQuery
{
    ...
    public class GetOverallScoreByLessonQueryHandler : IRequestHandler<GetOverallScoreByLessonQuery, MethodResult<OverallScoreReportModel>>
    {
        ...
        {
            var listScore = await _videoTimeCodeAnswerRepository.Queryable.Where(x => x.VideoResultId.HasValue && questionIds.Contains(x.QuestionId) && videoResultIds.Contains(x.VideoResultId.Value))
        ...
        private async Task<MethodResult<(List<Guid>, CourseResult)>> Validate(GetOverallScoreByLessonQuery request, CancellationToken cancellationToken)
        {
