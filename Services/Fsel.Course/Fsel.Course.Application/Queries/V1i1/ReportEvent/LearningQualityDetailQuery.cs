...
namespace Fsel.Course.Application.Queries.V1i1.ReportEvent
{
    ...
    public class LearningQualityDetailQuery : IRequest<MethodResult<IList<TotalDetailLearningQualityModel>>>
    {
    ...
    public class LearningQualityDetailQueryHandler : IRequestHandler<LearningQualityDetailQuery, MethodResult<IList<TotalDetailLearningQualityModel>>>
    {
        private readonly CourseDbContext _courseDbContext;
        private readonly ICacheService<IList<TotalDetailLearningQualityModel>> _cacheService;
        private readonly AppSetting _appSetting;
        private readonly AuthContext _authContext;

        public LearningQualityDetailQueryHandler(CourseDbContext courseDbContext,
                                                     ICacheService<IList<TotalDetailLearningQualityModel>> cacheService,
        ...
        public async Task<MethodResult<IList<TotalDetailLearningQualityModel>>> Handle(LearningQualityDetailQuery request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<IList<TotalDetailLearningQualityModel>>();

            var keyCache = $"LearningQualityDetailQuery_{ConvertHelper.Serialize(request)}_{_authContext.CurrentUserId}";
            ...
            var level = await _courseDbContext.Set<TotalDetailLearningQualityModel>()
                                              .FromSqlRaw("EXEC LearningQualityDetail @UserId, @DistrictIds, @GradeLevel, @SchoolIds, @TargetGroup, @Date",
