...
namespace Fsel.Course.Application.Queries.V1i1.ReportEvent
{
    ...
    public class RateLearningQualityQuery : IRequest<MethodResult<IList<RateLearningModel>>>
    {
    ...
    public class RateLearningQualityQueryHandler : IRequestHandler<RateLearningQualityQuery, MethodResult<IList<RateLearningModel>>>
    {
        private readonly CourseDbContext _courseDbContext;
        private readonly ICacheService<IList<RateLearningModel>> _cacheService;
        private readonly AppSetting _appSetting;
        private readonly AuthContext _authContext;

        public RateLearningQualityQueryHandler(CourseDbContext courseDbContext,
                                               ICacheService<IList<RateLearningModel>> cacheService,
        ...
        public async Task<MethodResult<IList<RateLearningModel>>> Handle(RateLearningQualityQuery request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<IList<RateLearningModel>>();

            var keyCache = $"RateLearningResults_{ConvertHelper.Serialize(request)}";
            ...
            var rate = await _courseDbContext.Database.SqlQueryRaw<RateLearningModel>("EXEC RateLearningResults @Target, @GroupByType, @DistrictIds, @GroupIds, @SchoolIds, @Date",
            ...
            methodResult.Result = rate;
            if (_appSetting.CacheConfig != null && _appSetting.CacheConfig.TurnOnCaching)
            {
                await _cacheService.SetAsync(keyCache, rate, TimeSpan.FromSeconds(_appSetting.CacheConfig.CachingDuration));
            }
