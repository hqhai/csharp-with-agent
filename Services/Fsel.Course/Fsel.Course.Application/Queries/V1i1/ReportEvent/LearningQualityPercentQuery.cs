...
namespace Fsel.Course.Application.Queries.V1i1.ReportEvent
{
    ...
    public class LearningQualityPercentQuery : IRequest<MethodResult<IList<RateLearningModel>>>
    {
    ...
    public class RateLearningQualityQueryHandler : IRequestHandler<LearningQualityPercentQuery, MethodResult<IList<RateLearningModel>>>
    {
        ...
        public async Task<MethodResult<IList<RateLearningModel>>> Handle(LearningQualityPercentQuery request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<IList<RateLearningModel>>();

            var keyCache = $"LearningQualityPercentQuery_{ConvertHelper.Serialize(request)}_{_authContext.CurrentUserId}";
            ...
            var level = await _courseDbContext.Set<RateLearningModel>()
                                              .FromSqlRaw("EXEC LearningQualityPercent @UserId, @DistrictIds, @GradeLevel, @SchoolIds, @TargetGroup, @Date",
