...
namespace Fsel.Course.Application.Queries.V1i1.ReportEvent
{
    ...
    public class LearningProgressAverageDetailQuery : IRequest<MethodResult<IList<AverageLearningProgressModel>>>
    {
    ...
    public class AverageLearningProgresQueryHandler : IRequestHandler<LearningProgressAverageDetailQuery, MethodResult<IList<AverageLearningProgressModel>>>
    {
        ...
        public async Task<MethodResult<IList<AverageLearningProgressModel>>> Handle(LearningProgressAverageDetailQuery request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<IList<AverageLearningProgressModel>>();

            var keyCache = $"LearningProgressAverageDetailQuery_{ConvertHelper.Serialize(request)}_{_authContext.CurrentUserId}";
            ...
            var level = await _courseDbContext.Set<AverageLearningProgressModel>()
                                              .FromSqlRaw("EXEC LearningProgressAverageDetail @UserId, @DistrictIds, @GradeLevel, @SchoolIds, @TargetGroup, @Date",
