...
namespace Fsel.Course.Application.Queries.V1i1.ReportEvent
{
    ...
    public class LearningProgressUnitDoneAcademicQuery : IRequest<MethodResult<IList<UnitDoneLearningProgressAcademicModel>>>
    {
    ...
    public class UnitDoneLearningProgressAcademicQueryHandler : IRequestHandler<LearningProgressUnitDoneAcademicQuery, MethodResult<IList<UnitDoneLearningProgressAcademicModel>>>
    {
        ...
        public async Task<MethodResult<IList<UnitDoneLearningProgressAcademicModel>>> Handle(LearningProgressUnitDoneAcademicQuery request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<IList<UnitDoneLearningProgressAcademicModel>>();

            var keyCache = $"LearningProgressUnitDoneAcademicQuery_{ConvertHelper.Serialize(request)}_{_authContext.CurrentUserId}";
            ...
            var level = await _courseDbContext.Set<UnitDoneLearningProgressAcademicModel>()
                                              .FromSqlRaw("EXEC LearningProgressUnitDoneAcademic @UserId, @DistrictIds, @GradeLevel, @SchoolIds, @TargetGroup, @Date",
