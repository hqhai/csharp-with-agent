...
namespace Fsel.Course.Application.Queries.V1i1.ReportEvent
{
    ...
    public class LearningQualityChartQuery : IRequest<MethodResult<IList<TotalLearningQualityModel>>>
    {
    ...
    public class LearningQualityChartQueryHandler : IRequestHandler<LearningQualityChartQuery, MethodResult<IList<TotalLearningQualityModel>>>
    {
        ...
        public LearningQualityChartQueryHandler(CourseDbContext courseDbContext,
        ...
        public async Task<MethodResult<IList<TotalLearningQualityModel>>> Handle(LearningQualityChartQuery request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<IList<TotalLearningQualityModel>>();

            var keyCache = $"LearningQualityChartQuery_{ConvertHelper.Serialize(request)}_{_authContext.CurrentUserId}";
            ...
            var level = await _courseDbContext.Set<TotalLearningQualityModel>()
                                              .FromSqlRaw("EXEC LearningQualityChart @UserId, @DistrictIds, @GradeLevel, @SchoolIds, @TargetGroup, @Date",
