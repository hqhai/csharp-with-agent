...
namespace Fsel.Course.Application.Queries.V1i1.ReportEvent
{
    ...
    public class InputEvaluationPercentQuery : IRequest<MethodResult<IList<PercentEvaluateInputResultModel>>>
    {
    ...
    public class InputEvaluationPercentQueryHandler : IRequestHandler<InputEvaluationPercentQuery, MethodResult<IList<PercentEvaluateInputResultModel>>>
    {
        ...
        public InputEvaluationPercentQueryHandler(CourseDbContext courseDbContext,
                                                      ICacheService<IList<PercentEvaluateInputResultModel>> cacheService,
        ...
        public async Task<MethodResult<IList<PercentEvaluateInputResultModel>>> Handle(InputEvaluationPercentQuery request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<IList<PercentEvaluateInputResultModel>>();

            var keyCache = $"InputEvaluationPercent_{ConvertHelper.Serialize(request)}_{_authContext.CurrentUserId}";
            ...
            var percent = await _courseDbContext.Set<PercentEvaluateInputResultModel>()
                                                .FromSqlRaw("EXEC InputEvaluationPercent @UserId, @DistrictIds, @GradeLevel, @SchoolIds, @TargetGroup, @Date",
            ...
            {
                await _cacheService.SetAsync(keyCache, percent, TimeSpan.FromSeconds(_appSetting.CacheConfig.CachingDuration));
            }
