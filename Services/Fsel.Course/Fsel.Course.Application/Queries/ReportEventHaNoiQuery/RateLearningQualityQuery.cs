...
namespace Fsel.Course.Application.Queries.ReportEventHaNoiQuery
{
    ...
    public class RateLearningQualityQueryHandler : IRequestHandler<RateLearningQualityQuery, MethodResult<IList<RateLearningModel>>>
    {
        ...
        public RateLearningQualityQueryHandler(CourseDbContext courseDbContext, ICacheService<IList<RateLearningModel>> cacheService)
        {
        ...
        public async Task<MethodResult<IList<RateLearningModel>>> Handle(RateLearningQualityQuery request, CancellationToken cancellationToken)
        {
            ...
            await _cacheService.SetAsync(keyCache, rate, TimeSpan.FromSeconds(CacheSettings.TimeCache.ThreeHour));
