        ...
        public async Task<MethodResult<Stream>> Handle(ImportStudentsIntoPlatformCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                foreach (var student in result.Datas.ToList())
                {
                    ...
                    {
                        var parent = await _userManager.Users.Include(p => p.Parent).FirstOrDefaultAsync(p => p.UserName == student.ParentEmail || p.Email == student.ParentEmail, cancellationToken);
                        if (parent == null)
                        {
                            parent = new User()
                            {
