...
namespace Fsel.Identity.Application.Queries.ReportEventHaNoiQuery
{
    ...
    public class AttendanceDetailQuery : IRequest<MethodResult<IList<SummaryDataOnCityModel>>>
    {
    ...
    public class AttendanceDetailQueryHandler : IRequestHandler<AttendanceDetailQuery, MethodResult<IList<SummaryDataOnCityModel>>>
    {
        ...
        public AttendanceDetailQueryHandler(UserDbContext userDbContext,
        ...
        public async Task<MethodResult<IList<SummaryDataOnCityModel>>> Handle(AttendanceDetailQuery request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<IList<SummaryDataOnCityModel>>();

            var keyCache = $"AttendanceDetailQuery_{ConvertHelper.Serialize(request)}_{_authContext.CurrentUserId}";
            ...
            var level = await _userDbContext.Set<SummaryDataOnCityModel>()
                                            .FromSqlRaw("EXEC AttendanceDetail @UserId, @DistrictIds, @GradeLevel, @SchoolIds, @TargetGroup, @Date",
