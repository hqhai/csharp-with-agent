...
namespace Fsel.System.Application.Commands.QuestBoardStudentCmd
{
    ...
    public class QuestBoardFinishOneHomeWorkCommand : QuestBoardStudentQueueModel, IRequest<MethodResult<bool>>
    {
    }

    public class QuestBoardFinishOneHomeWorkCommandHandler : IRequestHandler<QuestBoardFinishOneHomeWorkCommand, MethodResult<bool>>
    {
        ...
        public QuestBoardFinishOneHomeWorkCommandHandler(IQuestBoardRepository questBoardRepository
        ...
        public async Task<MethodResult<bool>> Handle(QuestBoardFinishOneHomeWorkCommand request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<bool>();

            var questBoardStudent = await _questBoardStudentRepository.Queryable.Include(x => x.QuestBoard)
                             .Where(x => x.QuestBoard != null && x.QuestBoard.Category == EnumQuestBoardCategory.FinishOneHomeworkMiniProject && x.Status == EnumQuestBoardStudentStatus.Process && x.StudentId == request.StudentId)
