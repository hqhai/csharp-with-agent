...
namespace Fsel.System.Application.Commands.QuestBoardStudentCmd
{
    ...
    public class QuestBoardFinishOneLessonCommand : QuestBoardStudentQueueModel, IRequest<MethodResult<bool>>
    {
    }

    public class QuestBoardFinishOneLessonCommandHandler : IRequestHandler<QuestBoardFinishOneLessonCommand, MethodResult<bool>>
    {
        ...
        public QuestBoardFinishOneLessonCommandHandler(IQuestBoardRepository questBoardRepository
        ...
        public async Task<MethodResult<bool>> Handle(QuestBoardFinishOneLessonCommand request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<bool>();

            var questBoardStudent = await _questBoardStudentRepository.Queryable.Include(x => x.QuestBoard)
                                .Where(x => x.QuestBoard != null && x.QuestBoard.Category == EnumQuestBoardCategory.FinishOneLesson && x.Status == EnumQuestBoardStudentStatus.Process && x.StudentId == request.StudentId)
