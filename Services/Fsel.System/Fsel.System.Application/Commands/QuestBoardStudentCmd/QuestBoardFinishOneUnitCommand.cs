...
namespace Fsel.System.Application.Commands.QuestBoardStudentCmd
{
    ...
    public class QuestBoardFinishOneUnitCommand : QuestBoardStudentQueueModel, IRequest<MethodResult<bool>>
    {
    }

    public class QuestBoardFinishOneUnitCommandHandler : IRequestHandler<QuestBoardFinishOneUnitCommand, MethodResult<bool>>
    {
        ...
        public QuestBoardFinishOneUnitCommandHandler(IQuestBoardRepository questBoardRepository
        ...
        public async Task<MethodResult<bool>> Handle(QuestBoardFinishOneUnitCommand request, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(request);
            var methodResult = new MethodResult<bool>();

            var questBoardStudent = await _questBoardStudentRepository.Queryable.Include(x => x.QuestBoard)
                             .Where(x => x.QuestBoard != null && x.QuestBoard.Category == EnumQuestBoardCategory.FinishOneUnit && x.Status == EnumQuestBoardStudentStatus.Process && x.StudentId == request.StudentId)
