...
namespace Fsel.ExamPractice.Infrastructure.Common
{
    ...
    using Fsel.ExamPractice.Domain.Entities.QuestionTypeConfigs.Answers;
    ...
    {
        public static (object?, short, bool, bool) GetTotalCorrectByAnswerType(object? configAnswer, object? configOldAnswer, Question question, bool isTryAgain = false, bool isSubmit = false, bool isMandatoryAnswer = false)
        {
        ...
        public static (object?, short, bool, bool) GetTotalCorrectByAnswerType(object? configAnswer, Question question, bool isMandatoryAnswer = false)
        {
            return GetTotalCorrectByAnswerType(configAnswer, null, question, false, false, isMandatoryAnswer);
        }

        public static short GetTotalCorrectByAnswerType(Question? question, object? configAnswer)
        {
            switch (question?.QuestionType)
            {
                case EnumQuestionType.MultichoiceV1:
        ...
        public static MethodResult<bool> ValidateAnswerLength(object? configAnswer, Question question)
        {
        ...
        public static object? AnswerTypeConverterObject(object? configAnswer, EnumQuestionType type, bool isShowSubStatus, EnumResultStatus status, bool isDisableAnswer = true)
        {
            object? result;
            switch (type)
            {
        ...
        private static bool IsAnswerMissing(object? dataAnswer, object? dataQuestion, EnumQuestionType type, bool isSubmit = false, bool isMandatoryAnswer = false)
        {
            ...
            switch (type)
            {
                ...
                case EnumQuestionType.Dropdown:
                case EnumQuestionType.Checklist:
                    return (!LinQAnswerHelper.CheckAnswerCount(dataAnswer, dataQuestion) || !LinQAnswerHelper.IsNullOrEmptyData(dataAnswer, nameof(MultipleChoiceAnswers.IsChecked), false));

                case EnumQuestionType.Listing:
                    return LinQAnswerHelper.IsNullOrEmptyData(dataAnswer);

                case EnumQuestionType.DragAndDropPicture:
                case EnumQuestionType.MatchingType1:
                case EnumQuestionType.MatchingType2:
                    return (!LinQAnswerHelper.CheckAnswerCount(dataAnswer, dataQuestion) || LinQAnswerHelper.IsNullOrEmptyData(dataAnswer, nameof(MatchingTypeAnswers.ToId)));
                ...
                case EnumQuestionType.GapFillScoreByGap:
                case EnumQuestionType.DragAndDropSentenceOrder:
                    return (!LinQAnswerHelper.CheckAnswerCount(dataAnswer, dataQuestion) || LinQAnswerHelper.IsNullOrEmptyData(dataAnswer, nameof(GapFillAnswers.Answer)));

                case EnumQuestionType.DragAndDropListSentenceOrder:
                    return !LinQAnswerHelper.CheckAnswerCount(dataAnswer, dataQuestion);

                case EnumQuestionType.MultipleOptionSentenceCompletion:
                    return LinQAnswerHelper.IsNullOrEmptyData(dataAnswer, nameof(MultipleOptionSentenceCompletionAnswers.AnswerId));
                ...
                case EnumQuestionType.FlowChartCompletion:
                case EnumQuestionType.TableCompletion:
                    return LinQAnswerHelper.IsDuplicateAnswerId(dataAnswer, nameof(ConfigAnswer.Id));
        ...
        private static (short, bool, bool) HandleCheckListAnswer(ref object? configAnswer, MultipleChoiceAnswer? dataOldAnswer, MultipleChoiceQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            short number = 0;
            ...
            configAnswer = dataAnswer;
            return (number > 0 ? number : default, isAnswerMissing, LinQAnswerHelper.IsNullOrEmptyData(dataAnswer.Answers, nameof(MultipleChoiceAnswers.IsChecked), false));
        }

        private static (short, bool, bool) HandleListingAnswer(ref object? configAnswer, ListingAnswer? dataOldAnswer, ListingQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            short number = 0;
            ...
            configAnswer = dataAnswer;
            return (number, isAnswerMissing, LinQAnswerHelper.IsAnswerHaveData(dataAnswer.Answers));
        }

        private static (short, bool, bool) HandleMaschingTypeAnswer(ref object? configAnswer, MatchingTypeAnswer? dataOldAnswer, MatchingTypeQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            ...
            configAnswer = dataAnswer;
            return (number, isAnswerMissing, LinQAnswerHelper.IsAnswerHaveData(dataAnswer.Answers, nameof(MatchingTypeAnswers.ToId)));
        }

        private static (short, bool, bool) HandleShortAnswerWordCount(ref object? configAnswer, ShortAnswerWordCountBaseAnswer? dataOldAnswer, ShortAnswerQuestionWordCountBaseQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
        ...
        private static (short, bool, bool) HandleShortAnswerWordBase(ref object? configAnswer, ShortAnswerWordBaseAnswer? dataOldAnswer, ShortAnswerQuestionWordBaseQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            ...
            if (dataQuestion.Content.Any(p => LinQAnswerHelper.IsQuestionContainedInAnswer(p, dataAnswer.Answers)))
            {
                dataAnswer.IsExact = true;
                number++;
            }
        ...
        private static (short, bool, bool) HandleGapFillBySubAnswer(ref object? configAnswer, GapFillAnswer? dataOldAnswer, GapFillQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            ...
            foreach (var item in dataAnswer.Answers)
            {
                ...
                {
                    item.IsExacts = item.Answer.Select((word, index) => LinQAnswerHelper.IsWordMatchAtIndex(question.Words, word, index)).ToList();
            ...
            configAnswer = dataAnswer;
            return (number, isAnswerMissing, LinQAnswerHelper.IsAnswerHaveData(dataAnswer.Answers, nameof(GapFillAnswers.Answer)));
        }

        private static (short, bool, bool) HandleGapFillGapAnswer(ref object? configAnswer, GapFillAnswer? dataOldAnswer, GapFillQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            var dataAnswer = configAnswer.Deserialize<GapFillAnswer>();
            short number = 0;
            bool isAnswerMissing = IsAnswerMissing(dataAnswer?.Answers, dataQuestion?.Contents, EnumQuestionType.GapFillScoreByQuestion, isSubmit, isMandatoryAnswer);
        ...
        private static (short, bool, bool) HandleDragDropOrderAnswer(ref object? configAnswer, DragAndDropSentenceOrderAnswer? dataOldAnswer, DragAndDropSentenceOrderQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            ...
            configAnswer = dataAnswer;
            return (number, isAnswerMissing, LinQAnswerHelper.IsAnswerHaveData(dataAnswer.Answers, nameof(DragAndDropSentenceOrderAnswers.Answer)));
        }

        private static (short, bool, bool) HandleAnswer(ref object? configAnswer, DragAndDropListSentenceOrderAnswer? dataOldAnswer, DragAndDropListSentenceOrderQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
        ...
        private static (short, bool, bool) HandleMultipleOptionAnswer(ref object? configAnswer, MultipleOptionSentenceCompletionAnswer? dataOldAnswer, MultipleOptionSentenceCompletionQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            ...
            configAnswer = dataAnswer;
            return (number, isAnswerMissing, LinQAnswerHelper.IsAnswerHaveData(dataAnswer.Answers, nameof(MultipleOptionSentenceCompletionAnswers.AnswerId)));
        }

        #region V1

        private static (short, bool, bool) HandleAnswerMultipleChoiceV1(ref object? configAnswer, MultipleChoiceAnswerV1? dataOldAnswer, MultipleChoiceQuestionV1? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            ...
            configAnswer = dataAnswer;
            return (number, isAnswerMissing, LinQAnswerHelper.IsAnswerHaveData(dataAnswer.Answers, nameof(ConfigAnswerV1.Key)));
        }

        private static (short, bool, bool) HandleAnswerCheckListV1(ref object? configAnswer, MultipleChoiceAnswerV1? dataOldAnswer, CheckListQuestionV1? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            var dataAnswer = configAnswer.Deserialize<MultipleChoiceAnswerV1>();
            short number = 0;
            ...
            configAnswer = dataAnswer;
            return (number > 0 ? number : default, isAnswerMissing, LinQAnswerHelper.IsAnswerHaveData(dataAnswer.Answers, nameof(ConfigAnswerV1.Key)));
        }

        private static (short, bool, bool) HandleAnswerSummaryCompletionGapFill(ref object? configAnswer, MultipleChoiceAnswerV1? dataOldAnswer, CheckListQuestionV1? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            var dataAnswer = configAnswer.Deserialize<MultipleChoiceAnswerV1>();
            short number = 0;
            ...
            foreach (var item in dataAnswer.Answers)
            {
                ...
                if (!string.IsNullOrEmpty(item.Content) && answerQuestion != null)
                {
                    ...
                    if (LinQAnswerHelper.CheckAnswer(answerQuestion.Content, item.Content))
                    {
                        number++;
                        item.IsExact = true;
                    }
            ...
            configAnswer = dataAnswer;
            return (number, isAnswerMissing, LinQAnswerHelper.IsAnswerHaveData(dataAnswer.Answers, nameof(ConfigAnswerV1.Content)));
        }

        private static (short, bool, bool) HandleAnswerTableCompletion(ref object? configAnswer, MultipleChoiceAnswerV1? dataOldAnswer, TableCompletionQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            var dataAnswer = configAnswer.Deserialize<MultipleChoiceAnswerV1>();
            short number = 0;
        ...
        private static (short, bool, bool) HandleAnswerMatchingTask(ref object? configAnswer, MultipleChoiceAnswerV1? dataOldAnswer, MatchingTaskQuestion? dataQuestion, bool isTryAgain, bool isSubmit, bool isMandatoryAnswer)
        {
            var dataAnswer = configAnswer.Deserialize<MultipleChoiceAnswerV1>();
            short number = 0;
            ...
            foreach (var item in dataAnswer.Answers)
            {
                ...
                {
                    if (LinQAnswerHelper.CheckAnswer(answerQuestion.Key, item.Key))
                    {
                        number++;
                        item.IsExact = true;
                    }
        ...
        public static object? GetConfigEmpty(EnumQuestionType type)
        {
            switch (type)
            {
                case EnumQuestionType.Multichoice:
