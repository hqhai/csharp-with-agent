...
RUN dotnet build "Fsel.Identity.Authentication.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "Fsel.Identity.Authentication.csproj" -c Release -o /app/publish /p:UseAppHost=false
